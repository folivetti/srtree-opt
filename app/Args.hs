...
import Data.SRTree.Opt ( Distribution (..) )
...
opt = Args
   ...
   <*> option distRead
        ...
        <> help "Minimize negative log-likelihood following one of\
                ...
                \ The default will use least squares to optimize the model."
        )
   <*> option s2Reader
       ( long "sErr"
       <> metavar "Serr"
...
distRead =
  str >>= \s -> mkReader ("unsupported distribution " <> s) Just (capitalize s)
  where
    capitalize ""     = ""
