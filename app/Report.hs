...
data BasicStats = Basic { _index   :: Int
                        , _fname   :: String
                        , _expr    :: Fix SRTree
                        , _nNodes  :: Int
                        , _nParams :: Int
...
getCI args dset basic alpha = (stats', cis, pis_tr, pis_val, pis_te)
  where
    ...
    prof th t = let (thOpt, _) = minimizeNLLWithFixedParam dist' msErr' 10 xTr yTr t 0 th
                 in case getProfile dist' msErr' xTr yTr t th stdErr tau_max 0 of
                      Left th' -> prof th' t
    ...
    pis_val   = case (_xVal dset, _yVal dset) of
                  (Nothing, _)           -> []
                  (_, Nothing)           -> []
                  (Just xVal, Just yVal) -> predictionCI method predFun jac prof xVal tree (VS.fromList theta) alpha
    pis_te    = case (_xTe dset, _yTe dset) of
                  (Nothing, _)           -> []
                  (_, Nothing)           -> []
                  (Just xTe, Just yTe) -> predictionCI method predFun jac prof xTe tree (VS.fromList theta) alpha